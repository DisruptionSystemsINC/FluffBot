...
public class VerifiedChecker extends ListenerAdapter {
    ...
    public void onGuildMemberJoin(GuildMemberJoinEvent event) {
        ...
        if (event.getGuild().getTextChannelsByName("to-verify", true).get(0).retrieveMessageById(event.getGuild().getTextChannelsByName("to-verify", true).get(0).getLatestMessageId()).complete().getAuthor() == event.getGuild().getSelfMember()) {
