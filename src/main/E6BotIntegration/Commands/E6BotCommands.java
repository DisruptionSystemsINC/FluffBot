...
public class E6BotCommands extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        ...
        switch (command) {
            case ("silly-media") -> {
                if (event.getChannel().asTextChannel() == event.getGuild().getTextChannelsByName("nsfw-bot", true).get(0)) {
            ...
            case ("media") -> {
                if (event.getChannel().asTextChannel() == event.getGuild().getTextChannelsByName("fluffymedia", true).get(0)) {
