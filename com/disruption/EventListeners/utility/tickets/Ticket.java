package disruption.EventListeners.utility.tickets;
...
public class Ticket{
    public static int createTicket(Member member, String DisplayName, StringSelectInteractionEvent event, String additionalMention, String type) throws IOException {
        ...
        if (!event.getGuild().getCategoriesByName(DisplayName + "s", true).toString().contains(DisplayName)) {
            ...
            channel.upsertPermissionOverride(member).grant(Permission.VIEW_CHANNEL, Permission.MESSAGE_SEND, Permission.MESSAGE_ATTACH_FILES).complete();
            if (member.hasPermission(Permission.VIEW_CHANNEL, Permission.MESSAGE_SEND, Permission.MESSAGE_ATTACH_FILES)) {
                try {
                ...
                event.reply("Das hat nicht Geklappt. Ein Fehler ist aufgetreten und das Ticket konnte nicht erstellt werden. Bitte versuche es nochmal oder kontaktiere GandhiTheDergRawr").setEphemeral(true).complete();
            }
        }
        return TicketID;
    }
